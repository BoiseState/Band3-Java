 ...
 *
 * ---------------------------
 * FastXYLineAndShapeRenderer.java
 * ---------------------------
 ...
 * Lines are not drawn if they connect two invisible data points, i.e. data points that are outside the axes ranges. The drawing of lines is reduced
 * to those lines that contribute to the visual appearance of the chart in way similar to that uses by the {@link SamplingXYLineRenderer}.
 */
public class FastXYLineAndShapeRenderer extends XYLineAndShapeRenderer
    ...
    protected void drawSeriesShapes(Graphics2D g2,
		...
		for(int itemIndex = itemCount - 1; itemIndex >= 0; itemIndex--){
            ...
            if(( x >= lowX ) && ( x <= highX )){ //Double.NaN won't come beyond this point
		        double y = dataset.getYValue(series, itemIndex);
		        if( ( y >= lowY ) && ( y <= highY ) ){ //Double.NaN won't come beyond this point
